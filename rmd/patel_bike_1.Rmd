...
df2 = left_join(neigh2,df1 %>% st_set_geometry(NULL)) %>% 
  mutate(blk_perc = pop_black_n/pop_n,
         covid_rate = total_positives/pop_n,
         wht_perc = pop_white_n/pop_n)
tm_shape(df2)+tm_polygons(c("adj_median_income","covid_rate","blk_perc","wht_perc"))

```

```{r}
#df2 %>% filter(objectid!=35) %>% tm_shape()+tm_polygons(c("adj_median_income","covid_rate","black_perc","wht_perc"),alpha=.4)
```
